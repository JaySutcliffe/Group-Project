...
class Arm:
    ...
    def collect_or_drop(self, x, y, pick_up):
        ...
        self.move_to(x, y, PIECE_HEIGHT, steps=4)
        self.move_claw(pick_up)
        self.move_to(x, y, 50 + PIECE_HEIGHT, steps=4)
        self.move_to(*REST_POS, steps=3)
    ...
    def move_to(self, x, y, z, steps=1):
        ...
        if steps > 1:
            ...
            y_step = (x - self.cart_pos[1]) / steps
            z_step = (x - self.cart_pos[2]) / steps
            for i in range(steps):
                # Move incrementally
                self.move_to(x + x_step * i, y + y_step * i, z * z_step * i, steps=1)
        else:
            # Actual movement
            ...
            self.server.send_pos(MOTOR_THETA, (theta + 180) * GEAR_THETA)
            self.server.send_pos(MOTOR_SHOULDER, shoulder * GEAR_SHOULDER)
            self.server.send_pos(MOTOR_ELBOW, elbow * GEAR_ELBOW)
            ...
            # Update positions
            self.angle_pos = (math.degrees(theta) + 180, math.degrees(shoulder), math.degrees(elbow))
