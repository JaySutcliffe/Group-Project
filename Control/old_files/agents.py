...
class PubevalAgent(Agent):
    ...
    wr = np.array([0.00000, -0.17160, 0.27010, 0.29906, -0.08471, 0.00000, -1.40375, -1.05121, 0.07217, -0.01351, 0.00000,
    ...
    wc = np.array([0.25696, -0.66937, -1.66135, -2.02487, -2.53398, -0.16092, -1.11725, -1.06654, -0.92830, -1.99558, -1.10388,
    ...
    def get_move(self, game, possible_moves):
        ...
        va = np.zeros(na)
        race = self.checkrace(game.get_pubeval_pos())
        for i in range(0, na):
            game.apply_move(possible_moves[i])
            va[i] = self.pubeval(race, game.get_pubeval_pos())
            game.apply_move(possible_moves[i], undo=True)
...
agents = [PubevalAgent(0), RandomAgent()]
wins1 = [0., 0.]
for _ in range(50):
    game = Game(agents)
    wins1[game.play()[0]] += 1
