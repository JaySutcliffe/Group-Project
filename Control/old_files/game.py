...
class Game:
    ...
    def get_pubeval_pos(self):
        features = [-self.bar[1]] #0
        for i in range(len(self.points)): #1-24
            point = self.points[23 - i]
            if point[0] != 0:
                ...
                features.append(0)
        features.append(self.bar[0]) #25
        features.append(self.off[0]) #26
        features.append(-self.off[1]) #27
